...
final class ArticleTypes {
    ...
    public function render_radio_metabox( \WP_Post $post, array $box = [] ): void {
        ...
        foreach ( self::terms() as $slug => $config ) {
            ...
            echo "<span class=\"smpi-article-type-text\"><span class=\"smpi-article-type-label\">" . esc_html( $config["label"] ) . "</span><span class=\"smpi-article-type-schema\">" . esc_html( $config["schema_type"] ) . "</span></span>";
            echo "</label>";
            echo "<div class=\"smpi-article-type-desc\">" . wp_kses_post( $details ) . "</div>";
            echo "</div>";
        }
        echo "</div>";
