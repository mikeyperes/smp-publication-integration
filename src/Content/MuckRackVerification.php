...
function normalizeTopBadges(){if(!data.authorHeaderBadge)return;var ct=contentTop();topAuthorTargets().forEach(function(el){if(ct!==null&&y(el)>ct)return;var root=authorRoot(el);q(".smpi-muckrack-icon,.smpi-muckrack-author-note",root).forEach(function(b){b.remove();});insertAfter(el,data.authorHeaderBadge,root);});}
