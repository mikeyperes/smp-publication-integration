...
var loopSelector=".e-loop-item,.elementor-loop-item,.elementor-post,.elementor-grid-item,.elementor-widget-loop-grid article,.elementor-posts-container article";
function isLoop(el){return !!(el&&el.closest(loopSelector));}
function containsLoop(el){return !!(el&&el.querySelector(loopSelector));}
...
function authorRoot(el){if(!el)return null;var root=el.closest(".elementor-post-info__item,.elementor-widget-post-info,.elementor-widget-theme-post-author,.elementor-author-box,.elementor-widget-author-box,.elementor-image-box-title,.elementor-icon-box-title,.elementor-heading-title,.elementor-widget-heading,.elementor-icon-list-item,.elementor-icon-list-text,.byline,.e-loop-item,.elementor-post,.elementor-grid-item");return root&&root!==document.body&&root!==document.documentElement?root:el.parentElement;}
function hasBadge(root){return !!(root&&root.querySelector(".smpi-muckrack-link,.smpi-muckrack-icon,.smpi-muckrack-author-note"));}
function isAuthorLink(el){return !!(el&&el.matches&&el.matches("a[href*='/author/'],a[rel='author']"));}
function isSemanticNameTarget(el){return !!(el&&(el.matches("h1,h2,h3,h4,h5,h6,.elementor-heading-title,.elementor-author-box__name,.elementor-image-box-title,.elementor-icon-box-title,.elementor-icon-list-text,[class*='author-name'],[class*='author_name'],[itemprop='name'],.byline")||el.closest("h1,h2,h3,h4,h5,h6,.elementor-heading-title,.elementor-author-box__name,.elementor-image-box-title,.elementor-icon-box-title,.elementor-icon-list-text,[class*='author-name'],[class*='author_name'],[itemprop='author'],[itemprop='name'],.byline")));}
function isProseNameTarget(el){if(!el||isSemanticNameTarget(el))return false;return !!el.closest(".smpi-author-bio,.elementor-author-box__bio,.elementor-author-box__text,.elementor-post__excerpt,.elementor-widget-text-editor,.elementor-widget-theme-post-content,.elementor-widget-post-content,.entry-content,.post-content,p,li,blockquote");}
function exactAuthorTarget(el){if(!el)return null;if(el.matches("a[href]"))return el;var link=el.closest("a[href]");return link&&(isAuthorLink(link)||(isSemanticNameTarget(el)&&norm(link.textContent)===norm(el.textContent)))?link:el;}
...
function nameTargetScore(el){if(!el)return -1;var score=0;var link=el.matches("a[href]")?el:el.closest("a[href]");if(isAuthorLink(link))score+=1000;if(el.closest(".elementor-author-box__name,[class*='author-name'],[class*='author_name'],[itemprop='author'],[itemprop='name']"))score+=900;if(el.closest("h1,h2,h3,h4,h5,h6,.elementor-heading-title"))score+=800;if(el.closest(".elementor-image-box-title,.elementor-icon-box-title"))score+=700;if(el.closest(".elementor-icon-list-text,.byline"))score+=600;if(!isProseNameTarget(el))score+=100;return score;}
function sortNameTargets(nodes){return unique(nodes).sort(function(a,b){var score=nameTargetScore(b)-nameTargetScore(a);if(score)return score;var ay=y(a),by=y(b);if(ay!==by)return ay-by;return a.getBoundingClientRect().height-b.getBoundingClientRect().height;});}
function exactTextTargets(root,name){var target=norm(name);if(!root||!target)return [];var out=[];var selectors="a[href*='/author/'],a[rel='author'],.elementor-heading-title,.elementor-heading-title *,.elementor-author-box__name,.elementor-author-box__name *,.elementor-image-box-title,.elementor-image-box-title *,.elementor-icon-box-title,.elementor-icon-box-title *,.elementor-icon-list-text,.elementor-icon-list-text *,[class*='author-name'],[class*='author-name'] *,[class*='author_name'],[class*='author_name'] *,[itemprop='author'] [itemprop='name'],[itemprop='name'],h1,h2,h3,h4,h5,h6,h1 *,h2 *,h3 *,h4 *,h5 *,h6 *,.byline,.byline *";q(selectors,root).forEach(function(el){if(!visible(el)||isAdminOrHidden(el)||isInvalidPlacement(el)||isProseNameTarget(el))return;if(norm(el.textContent)!==target)return;var childExact=Array.prototype.some.call(el.children||[],function(ch){return norm(ch.textContent)===target&&!isProseNameTarget(ch);});if(childExact)return;out.push(el);});return sortNameTargets(out);}
function structuralAuthorLinks(root){return unique(q("a[href*='/author/'],a[rel='author']",root).filter(function(el){return visible(el)&&clean(el.textContent)!==""&&!isAdminOrHidden(el)&&!isInvalidPlacement(el)&&!isProseNameTarget(el);}));}
function bestNameTarget(root,name){var target=norm(name);var out=structuralAuthorLinks(root).filter(function(el){return norm(el.textContent)===target;}).concat(exactTextTargets(root,name));var sorted=sortNameTargets(out);return sorted.length?sorted[0]:null;}
function contentBottom(){var el=contentWidget();return el?y(el)+el.getBoundingClientRect().height:null;}
function topAuthorTargets(){var ct=contentTop();var target=norm(data.authorName);var out=structuralAuthorLinks(document).filter(function(el){if(isLoop(el)||(ct!==null&&y(el)>ct))return false;return !target||norm(el.textContent)===target;});exactTextTargets(document,data.authorName).forEach(function(el){if(!isLoop(el)&&(ct===null||y(el)<=ct))out.push(el);});var sorted=sortNameTargets(out);return sorted.length?[sorted[0]]:[];}
function authorCardContainers(){var floor=contentBottom();var out=[];q(".elementor-author-box,.elementor-widget-theme-post-author,.elementor-widget-author-box,[class~='author-box'],[class~='about-author'],.smpi-author-card").forEach(function(el){var root=el.closest(".elementor-widget-theme-post-author,.elementor-widget-author-box,.elementor-element,[class~='author-box'],[class~='about-author']")||el;if(visible(root)&&!isLoop(root)&&!containsLoop(root)&&!isAdminOrHidden(root)&&(floor===null||y(root)>=floor-2))out.push(root);});q(".e-con,.elementor-section,.elementor-container,.elementor-element").forEach(function(el){if(!visible(el)||isLoop(el)||containsLoop(el)||isAdminOrHidden(el)||(floor!==null&&y(el)<floor-2))return;var rect=el.getBoundingClientRect();if(rect.height<60||rect.height>900||rect.width<120)return;var target=bestNameTarget(el,data.authorName);if(!target)return;var hasProfileVisual=!!el.querySelector("img,.elementor-widget-image,.elementor-icon-box-icon,.elementor-image-box-img");var hasAuthorLink=!!el.querySelector("a[href*='/author/'],a[rel='author']");var hasBio=!!el.querySelector(".smpi-author-bio,.elementor-author-box__bio,.elementor-widget-text-editor");var tx=clean(el.textContent).toLowerCase();var hasAbout=tx.indexOf("about the author")!==-1;var hasSocial=/twitter\s*\/\s*x|linkedin|instagram|email/.test(tx);if(hasProfileVisual&&(hasAuthorLink||hasBio||hasAbout||hasSocial))out.push(el);});return unique(out).sort(function(a,b){return a.getBoundingClientRect().height-b.getBoundingClientRect().height;});}
function footerAuthorTargets(){var cards=authorCardContainers();for(var i=0;i<cards.length;i++){var target=bestNameTarget(cards[i],data.authorName);if(target)return [{el:target,root:cards[i]}];}var floor=contentBottom();var fallback=exactTextTargets(document,data.authorName).filter(function(el){return !isLoop(el)&&(floor===null||y(el)>=floor-2);});return fallback.length?[{el:fallback[0],root:authorRoot(fallback[0])}]:[];}
...
function cleanupInvalidBadges(){q(".smpi-muckrack-icon").forEach(function(icon){var footerInLoop=icon.classList.contains("smpi-muckrack-context-single_footer")&&isLoop(icon);if(isInvalidPlacement(icon)||footerInLoop||isProseNameTarget(icon))removeBadge(icon);});}
