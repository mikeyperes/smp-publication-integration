...
class AjaxController {
    ...
    public function save_settings( AjaxRequest $request ): array {
        ...
        $boolean_keys[] = "external_publishing_enabled";
        ...
        foreach ( [ "author_social_size" => [ 12, 64, 24 ], "muckrack_icon_size" => [ 8, 64, 16 ], "muckrack_verified_font_size" => [ 8, 64, 14 ], "publication_muckrack_font_size" => [ 8, 64, 14 ], "breadcrumbs_font_size" => [ 8, 64, 13 ], "table_of_contents_text_font_size" => [ 8, 64, 15 ], "article_heading_h2_font_size" => [ 8, 64, 23 ], "article_heading_h3_font_size" => [ 8, 64, 20 ], "article_numbered_list_font_size" => [ 8, 64, 16 ], "article_drop_cap_font_size" => [ 48, 180, 96 ], "inline_photo_caption_font_size" => [ 8, 64, 16 ], "featured_image_caption_font_size" => [ 8, 64, 16 ], "post_summary_font_size" => [ 8, 64, 16 ], "post_faqs_text_font_size" => [ 8, 64, 16 ], "muckrack_icon_size_single_author" => [ 0, 64, 0 ], "muckrack_icon_size_single_footer" => [ 0, 64, 0 ], "muckrack_icon_size_loop_cards" => [ 0, 64, 0 ], "muckrack_icon_size_home" => [ 0, 64, 0 ], "muckrack_icon_size_author" => [ 0, 64, 0 ] ] as $key => $limits ) {
            if ( $request->has( $key, 'post' ) ) {
        ...
        foreach ( [ "author_social_style", "author_social_archive_position", "author_archive_loading_mode", "author_archive_loading_style", "breadcrumbs_style", "table_of_contents_style", "article_heading_style", "article_numbered_list_style", "article_drop_cap_style", "inline_photo_treatment", "featured_image_caption_template", "post_summary_style", "post_summary_background_mode", "post_summary_placement", "post_faqs_style", "post_faqs_placement", "multi_authors_loop_output", "table_of_contents_text_font_style", "inline_photo_caption_font_style", "featured_image_caption_font_style", "post_faqs_text_font_style", 'post_time_mode', 'muckrack_verified_style', 'muckrack_icon_style', 'publication_muckrack_text_mode', 'publication_muckrack_style' ] as $key ) {
            if ( $request->has( $key, 'post' ) ) {
                $changes[ $key ] = $request->key( $key, '', 'post' );
            }
        }
        foreach ( [ 'author_social_color', 'muckrack_icon_color', 'muckrack_icon_color_single_author', 'muckrack_icon_color_single_footer', 'muckrack_icon_color_loop_cards', 'muckrack_icon_color_home', 'muckrack_icon_color_author', 'muckrack_verified_text_color', 'breadcrumbs_accent_color', 'breadcrumbs_background_color', 'breadcrumbs_text_color', 'table_of_contents_accent_color', 'table_of_contents_text_color', 'article_heading_accent_color', 'article_heading_text_color', 'article_numbered_list_accent_color', 'article_numbered_list_text_color', 'article_drop_cap_color', 'inline_photo_accent_color', 'inline_photo_caption_text_color', 'featured_image_caption_accent_color', 'featured_image_caption_text_color', 'post_summary_accent_color', 'post_summary_background_color', 'post_summary_text_color', 'post_faqs_accent_color', 'post_faqs_text_color', 'publication_muckrack_color', 'publication_muckrack_text_color' ] as $color_key ) {
            if ( $request->has( $color_key, 'post' ) ) {
                $raw = trim( (string) $request->raw( $color_key, '', 'post' ) );
                $changes[ $color_key ] = '' === $raw ? '' : sanitize_hex_color( $raw );
            }
        }
        foreach ( [ "author_social_networks", "author_social_auto_contexts", "muckrack_verified_contexts", "publication_muckrack_placements", "press_release_include_contexts", "breadcrumbs_disabled_post_types", "post_hygiene_allowed_post_types" ] as $array_key ) {
