...
class DashboardController {
    ...
    private function features(): void {
        ...
        foreach ( [ "font_family", "font_size", "font_color", "font_weight" ] as $property ) {
            if ( Settings::bool( "article_heading_preserve_" . $property ) ) {
                $design_classes[] = "smpi-preserve-heading-" . str_replace( "_", "-", $property );
            }
        }
        echo "<div class=\"" . esc_attr( implode( " ", $design_classes ) ) . "\" style=\"" . esc_attr( $this->design_font_variables() ) . "\">";
    ...
    private function render_article_design_feature_group( array $settings ): void {
        ...
        $article_heading_controls = "<div class=\"smpi-control-group smpi-heading-preserve-controls\"><h3>Keep current heading settings</h3><p class=\"description\">Preserve the active theme or Elementor typography while applying the selected design.</p>"
            . $this->inline_toggle_setting_html( "article_heading_preserve_font_family", "Leave font as is" )
            . $this->inline_toggle_setting_html( "article_heading_preserve_font_size", "Leave font size as is" )
            . $this->inline_toggle_setting_html( "article_heading_preserve_font_color", "Leave font color as is" )
            . $this->inline_toggle_setting_html( "article_heading_preserve_font_weight", "Leave font weight as is" )
            . "</div>"
        ...
        $drop_cap_controls = $this->select_setting_html( "article_drop_cap_style", $this->article_drop_cap_style_options(), $settings, "First-letter template" )
            . $this->select_setting_html( "article_drop_cap_script_font", $this->article_drop_cap_script_font_options(), $settings, "Script letter font" )
            . $this->font_family_setting_html( "article_drop_cap_font_family", "Drop cap font", $settings )
...
HTML;
    ...
    private function quick_run(): void {
        ...
        QuickStartCleanupWorkflow::render_assets();
    }
    ...
    private function snippets(): void {
        ...
        echo "<div class=\"smpi-section-intro\"><h2>Snippet implementation catalog</h2><p>Feature controls live in Editorial &gt; Features. This view contains implementation metadata and test status.</p></div>";
        echo ( new SnippetsTableRenderer() )->render( $registry, [
            "title"         => "Implementations",
            "description"   => "Hooks, code references, tests, and operational notes.",
...
CSS;
    ...
    private function font_family_setting_html( string $key, string $label, array $settings ): string {
    ...
    private function design_font_variables(): string {
        ...
        foreach ( Settings::font_family_css_variables() as $key => $variable ) {
            if ( "article_heading_font_family" === $key && Settings::bool( "article_heading_preserve_font_family" ) ) {
                continue;
            }
        ...
        foreach ( Settings::font_weight_css_variables() as $key => $variable ) {
            if ( "article_heading_font_weight" === $key && Settings::bool( "article_heading_preserve_font_weight" ) ) {
                continue;
            }
    ...
    private function article_drop_cap_script_font_options(): array {
        $descriptions = [
            "dancing-script" => "Fluid, readable strokes with a balanced editorial feel.",
            "great-vibes" => "Ornate high-contrast strokes for the most dramatic opening.",
            "parisienne" => "Light, relaxed handwriting with compact flourishes.",
            "pinyon-script" => "Formal calligraphy with long, elegant letterforms.",
        ];
        $options = [];
        foreach ( \smp_publication_integration\Content\ArticleStyles::article_drop_cap_script_fonts() as $key => $font ) {
            $options[ $key ] = [
                "label" => $font["label"],
                "description" => $descriptions[ $key ] ?? "Script letter style.",
                "preview" => "<span class=\"smpi-script-font-preview smpi-script-font-preview--" . esc_attr( $key ) . "\">A graceful editorial opening</span>",
            ];
        }
    ...
    private function article_heading_style_options(): array {
        return [
            ...
            "h2-tick" => [ "label" => "H2 01: Accent Tick", "description" => "Short accent bar before the heading.", "preview" => $this->article_heading_preview_html( "h2-tick" ) ],
            "h2-leftrule" => [ "label" => "H2 02: Left Rule", "description" => "Vertical accent bar, echoing the What to Know block.", "preview" => $this->article_heading_preview_html( "h2-leftrule" ) ],
            "h2-underline" => [ "label" => "H2 03: Short Underline", "description" => "Compact accent underline anchored left.", "preview" => $this->article_heading_preview_html( "h2-underline" ) ],
            "h2-topline" => [ "label" => "H2 04: Top Hairline", "description" => "Full hairline above with generous breathing room.", "preview" => $this->article_heading_preview_html( "h2-topline" ) ],
            "h2-dot" => [ "label" => "H2 05: Lead Dot", "description" => "Small accent dot leads the heading.", "preview" => $this->article_heading_preview_html( "h2-dot" ) ],
            "h2-trailingrule" => [ "label" => "H2 06: Trailing Rule", "description" => "Heading followed by a line that fills the row.", "preview" => $this->article_heading_preview_html( "h2-trailingrule" ) ],
            "h2-serif" => [ "label" => "H2 07: Serif + Tick", "description" => "Serif heading with a thin accent tick.", "preview" => $this->article_heading_preview_html( "h2-serif" ) ],
            "h2-uppercase" => [ "label" => "H2 08: Uppercase Eyebrow", "description" => "Letter-spaced caps over a short accent rule.", "preview" => $this->article_heading_preview_html( "h2-uppercase" ) ],
            "h2-gradient" => [ "label" => "H2 09: Gradient Underline", "description" => "Accent-to-transparent underline.", "preview" => $this->article_heading_preview_html( "h2-gradient" ) ],
            "h2-bracket" => [ "label" => "H2 10: Corner Bracket", "description" => "Small accent bracket frames the start.", "preview" => $this->article_heading_preview_html( "h2-bracket" ) ],
            "h2-number" => [ "label" => "H2 11: Indexed", "description" => "Auto-numbered accent index before the heading.", "preview" => $this->article_heading_preview_html( "h2-number" ) ],
            "h2-square" => [ "label" => "H2 12: Accent Square", "description" => "Rounded accent square leads in.", "preview" => $this->article_heading_preview_html( "h2-square" ) ],
            "h2-highlight" => [ "label" => "H2 13: Marker Highlight", "description" => "Subtle accent wash under the text.", "preview" => $this->article_heading_preview_html( "h2-highlight" ) ],
            "h2-double" => [ "label" => "H2 14: Framed Rules", "description" => "Heading set between an accent rule and a hairline.", "preview" => $this->article_heading_preview_html( "h2-double" ) ],
            "h2-corner_tick" => [ "label" => "H2 15: Standing Tick", "description" => "Short vertical accent tick above-left.", "preview" => $this->article_heading_preview_html( "h2-corner_tick" ) ],
        ];
    }

    private function article_heading_preview_sample( string $style ): array {
        $samples = [
            "h2-tick" => [ "Where the method began", "This section walks through how the idea first took shape and why it connected with early customers." ],
            "h2-leftrule" => [ "Building the framework", "A repeatable structure replaced improvisation, giving the team something concrete to lean on." ],
            "h2-underline" => [ "A practical foundation", "The early groundwork mattered more than the origin story, and it shows in the results." ],
            "h2-topline" => [ "Scaling the approach", "What worked for one seller had to be translated into a system that worked for many." ],
            "h2-dot" => [ "What changed for sellers", "Confidence followed competence once the fundamentals were taught in the right order." ],
            "h2-trailingrule" => [ "Inside the training", "Each module built on the last, moving from listening skills to live objection handling." ],
            "h2-serif" => [ "From classroom to call floor", "The instincts that make a good teacher turn out to make a good closer too." ],
            "h2-uppercase" => [ "The core principle", "Sell by understanding the person in front of you, not by performing enthusiasm." ],
            "h2-gradient" => [ "Reading the room", "Timing and attention do more for a conversation than any single script ever could." ],
            "h2-bracket" => [ "Handling objections", "Resistance is information; the method treats it as a step rather than a setback." ],
            "h2-number" => [ "Structure over hype", "Process, not personality, is what keeps results steady across very different sellers." ],
            "h2-square" => [ "Coaching the team", "Feedback loops turned individual wins into habits the whole group could repeat." ],
            "h2-highlight" => [ "Why it resonated", "People wanted a method they could trust, and a method is exactly what they got." ],
            "h2-double" => [ "The bigger shift", "The change was less about tactics and more about how sales itself was being taught." ],
            "h2-corner_tick" => [ "What comes next", "The next chapter is about reach - bringing the same clarity to a much wider audience." ],
        ];
    ...
    private function article_heading_report_html(): string {
        ...
        $preserved = [];
        foreach ( [ "font_family" => "font", "font_size" => "size", "font_color" => "color", "font_weight" => "weight" ] as $key => $label ) {
            if ( Settings::bool( "article_heading_preserve_" . $key ) ) {
                $preserved[] = $label;
            }
        }
    ...
    private function article_drop_cap_report_html(): string {
        ...
        $script_font = \smp_publication_integration\Content\ArticleStyles::article_drop_cap_script_font_data( (string) Settings::get( "article_drop_cap_script_font", "dancing-script" ) );
    ...
    private function post_acf_addons_report_html(): string {
        ...
        return $html . "</tbody></table><p class=smpi-muted>Summary font: " . esc_html( Settings::font_family_label( "post_summary_font_family" ) ) . ". FAQ font: " . esc_html( Settings::font_family_label( "post_faqs_font_family" ) ) . ".</p>";
    }
...
[acf_author_field field=\"job_title\" author_index=\"1\"]", $this->multi_authors_report_html(), $this->activity_log_html(), $this->multi_author_controls_html( $settings ), false );
    ...
    private function multi_author_loop_output_controls_html( array $settings ): string {
        return $this->select_setting_html(
            "multi_authors_loop_output",
            [
                "comma" => [
    ...
    private function scripts(): void {
        ...
        jQuery(function($){
            ...
            var smpiScriptFonts=<?php echo wp_json_encode( \smp_publication_integration\Content\ArticleStyles::article_drop_cap_script_fonts() ); ?>;
            function smpiSyncHeadingPreserveControls(){var host=document.querySelector(`.smpi-design-host`),controls={font_family:$(`[data-key="article_heading_font_family"]`),font_size:$(`[data-key="article_heading_h2_font_size"],[data-key="article_heading_h3_font_size"]`),font_color:$(),font_weight:$(`[data-key="article_heading_font_weight"]`)};$.each(controls,function(property,targets){var toggle=$(`[data-key="article_heading_preserve_${property}"]`).first(),preserve=toggle.is(`:checked`),className=`smpi-preserve-heading-`+property.replace(/_/g,`-`);if(host)host.classList.toggle(className,preserve);targets.prop(`disabled`,preserve).attr(`aria-disabled`,preserve?`true`:`false`).closest(`.smpi-control-group`).toggleClass(`is-preserved`,preserve);if(!host)return;if(property===`font_family`||property===`font_weight`){var variable=property===`font_family`?`--smpi-heading-font`:`--smpi-heading-weight`;if(preserve){host.style.removeProperty(variable)}else{var selected=targets.get(0),option=selected&&selected.options?selected.options[selected.selectedIndex]:null,css=option?String(option.getAttribute(`data-css`)||``):``;if(css)host.style.setProperty(variable,css);else host.style.removeProperty(variable)}}})}
            $(document).on(`change`,`[data-key^="article_heading_preserve_"]`,smpiSyncHeadingPreserveControls);
            smpiSyncHeadingPreserveControls();
            $(document).on(`input change`,`.smpi-setting`,function(){var k=$(this).data(`key`),host=document.querySelector(`.smpi-design-host`);if(k===`article_drop_cap_script_font`&&host){var script=smpiScriptFonts[$(this).val()];if(script){host.style.setProperty(`--smpi-dropcap-script-font`,JSON.stringify(String(script.family||`Dancing Script`)));host.style.setProperty(`--smpi-dropcap-script-weight`,String(script.default_weight||`600`))}}var m=smpiPV[k];if(!m)return;if(host){smpiSetPreviewVar(host,m[0],$(this).val(),m[1]);smpiSetDerivedPreviewVars(host,k,$(this).val())}});
    ...
    private function styles(): void { ?>
        ...
        <style id="smpi-dropcap-script-font-admin-css">.smpi-control-group:has(input[data-key="article_drop_cap_script_font"]) .smpi-choice-grid{grid-template-columns:1fr}.smpi-control-group:has(input[data-key="article_drop_cap_script_font"]) .smpi-choice-preview{max-width:640px}.smpi-script-font-preview{color:#111827;display:block;font-size:42px;letter-spacing:0;line-height:1.2;overflow-wrap:anywhere;padding:4px 2px}</style>
        ...
        <style id="smpi-font-family-preview-css">
            ...
            .smpi-design-host:not(.smpi-preserve-heading-font-family) .smpi-article-heading{font-family:var(--smpi-heading-font)!important}
            .smpi-design-host:not(.smpi-preserve-heading-font-weight) .smpi-article-heading{font-weight:var(--smpi-heading-weight,700)!important}
            .smpi-design-host:not(.smpi-preserve-heading-font-color) .smpi-article-heading{color:var(--smpi-heading-ink,#111827)!important}
            .smpi-design-host:not(.smpi-preserve-heading-font-size) .smpi-article-heading--h2{font-size:var(--smpi-heading-h2-size,23px)!important}
            .smpi-design-host:not(.smpi-preserve-heading-font-size) .smpi-article-heading--h3{font-size:var(--smpi-heading-h3-size,20px)!important}
            .smpi-design-host .smpi-dropcap-preview .smpi-article-lead::first-letter{font-family:var(--smpi-dropcap-font)!important}
            .smpi-design-host .smpi-dropcap-preview .smpi-article-lead::first-letter{font-weight:var(--smpi-dropcap-weight)!important}
