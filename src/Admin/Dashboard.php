...
final class Dashboard {
    ...
    private function features(): void {
        ...
        $author_muckrack_controls = $this->context_select_html( "muckrack_verified_contexts", [ "single_author" => "single.php header author mention", "single_footer" => "single.php footer mention", "home" => "Home page author mention", "author" => "author.php author mention" ], $settings, "Placement contexts" ) . $this->select_setting_html( "muckrack_verified_style", [ "tooltip" => "Tooltip icon", "text" => "Inline text" ], $settings, "Display style" ) . $this->icon_style_setting_html( $settings ) . $this->color_setting_html( "muckrack_icon_color", "Icon color", $settings ) . $this->inline_toggle_setting_html( "muckrack_author_always_show", "Always show for every author" );
    ...
    private function context_select_html( string $key, array $options, array $settings, string $label = "Placement contexts" ): string {
        ...
        foreach ( $options as $value => $option_label ) {
            $html .= "<label class=smpi-choice-card><input class=smpi-setting-array type=checkbox data-key=" . esc_attr( $key ) . " value=" . esc_attr( $value ) . " " . checked( in_array( $value, $selected, true ), true, false ) . "><span>" . esc_html( $option_label ) . "</span></label>";
        }
    ...
    private function select_setting_html( string $key, array $options, array $settings, string $label = "Style" ): string {
        ...
        $html = "<div class=smpi-control-group><h3>" . esc_html( $label ) . "</h3><div class=smpi-choice-grid>";
        foreach ( $options as $value => $option_label ) {
            $html .= "<label class=smpi-choice-card><input class=smpi-setting type=radio name=" . esc_attr( "smpi_" . $key ) . " data-key=" . esc_attr( $key ) . " value=" . esc_attr( $value ) . " " . checked( $current, $value, false ) . "><span>" . esc_html( $option_label ) . "</span></label>";
        }
        return $html . "</div><span class=spinner></span><span class=smpi-save-state></span></div>";
    }

    private function color_setting_html( string $key, string $label, array $settings ): string {
        $color = sanitize_hex_color( (string) ( $settings[ $key ] ?? "#2d5277" ) ) ?: "#2d5277";
        return "<div class=smpi-control-group><h3>" . esc_html( $label ) . "</h3><label class=smpi-color-control><input class=smpi-setting type=color data-key=" . esc_attr( $key ) . " value=" . esc_attr( $color ) . "><code>" . esc_html( $color ) . "</code></label><span class=spinner></span><span class=smpi-save-state></span></div>";
    }

    private function icon_style_setting_html( array $settings ): string {
        return $this->select_setting_html( "muckrack_icon_style", [ "circle_check" => "Circle with check", "check" => "Just check" ], $settings, "Icon style" ) . "<div class=smpi-icon-preview><span class=smpi-muckrack-preview-circle>&#10003;</span><span class=smpi-muckrack-preview-check>&#10003;</span></div>";
    }
