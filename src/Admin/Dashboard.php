...
[muckrack_verified type=\"icon\" user_id=\"54\"]", $this->muckrack_report_html(), $this->activity_log_html(), $author_muckrack_controls );
    ...
    private function author_context_overrides_html( array $settings ): string {
        ...
        $html = "<div class=\"smpi-control-group smpi-context-overrides\"><h3>Context overrides</h3><p class=smpi-muted>Leave color blank and size at 0 to inherit the defaults above.</p><div class=smpi-context-override-list>";
        foreach ( $contexts as $context => $meta ) {
            ...
            $html .= "<div class=smpi-context-override-row><div><strong>" . esc_html( $meta["label"] ) . "</strong><small>" . esc_html( $meta["description"] ) . "</small></div><label>Color override <input class=smpi-setting type=text inputmode=text pattern=\"^#?[0-9a-fA-F]{0,6}$\" placeholder=\"inherit\" data-key=" . esc_attr( $color_key ) . " value=\"" . esc_attr( $color ) . "\"></label><label>Size override <input class=smpi-setting type=number min=0 max=64 data-key=" . esc_attr( $size_key ) . " value=\"" . esc_attr( (string) $size ) . "\"><span>px</span></label></div>";
        }
        return $html . "</div><span class=spinner></span><span class=smpi-save-state></span></div>";
    }
