...
[smp_post_authors context=\"card\" format=\"lines\"]", $this->multi_authors_report_html(), $this->activity_log_html(), $multi_author_controls );
    ...
    private function article_heading_preview_html( string $style ): string {
        ...
        $heading_class = "none" === $style ? "" : " class=\"" . esc_attr( $style ) . "\"";
        $sample = $this->article_heading_preview_sample( $style );
        return "<div class=\"smpi-ah-preview-stack\"><div class=\"" . esc_attr( $class ) . "\"><h2" . $heading_class . ">" . esc_html( $sample[0] ) . "</h2><p>" . esc_html( $sample[1] ) . "</p></div><div class=\"" . esc_attr( $class ) . "\"><h3" . $heading_class . ">What changed next</h3><p>The same selected treatment applies to article H3 subheadings inside post content.</p></div></div>";
    }
...
[acf_author_field field=\"job_title\" author_index=\"1\"]", $this->multi_authors_report_html(), $this->activity_log_html(), $this->multi_author_controls_html( $settings ) );
    ...
    private function styles(): void { ?>
        ...
        <style id="smpi-article-heading-admin-css">.smpi-control-group:has(input[data-key="article_heading_style"]) .smpi-choice-grid{grid-template-columns:1fr}.smpi-control-group:has(input[data-key="article_heading_style"]) .smpi-choice-preview{background:transparent;border:0;max-width:820px;padding:0}.smpi-control-group:has(input[data-key="article_heading_style"]) .smpi-choice-card{min-height:0}.smpi-choice-preview .smpi-ah-preview h2,.smpi-choice-preview .smpi-ah-preview h3{text-transform:none;letter-spacing:0}</style>
