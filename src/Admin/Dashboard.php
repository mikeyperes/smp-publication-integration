...
final class Dashboard {
    ...
    public function render(): void {
        ...
        <div class="wrap smpi-dashboard">
            ...
            <?php $this->styles(); ?>
            <div class="smpi-tabs-nav" role="tablist"><?php foreach ( $tabs as $id => $label ) : $tab_url = admin_url( "options-general.php?page=" . Config::$settings_page_slug . "&tab=" . $id ); ?><a class="smpi-tab-btn<?php echo $id === $active ? " active" : ""; ?>" href="<?php echo esc_url( $tab_url ); ?>" data-tab="<?php echo esc_attr( $id ); ?>" role="tab" aria-selected="<?php echo $id === $active ? "true" : "false"; ?>"><?php echo esc_html( $label ); ?></a><?php endforeach; ?></div>
            <div class="smpi-tab-status" aria-live="polite"><span class="spinner"></span><span class="smpi-tab-message"></span></div>
            <section id="smpi-tab-panel" data-active-tab="<?php echo esc_attr( $active ); ?>" class="smpi-tab-content active" aria-live="polite"><?php $this->tab( $active ); ?></section>
        </div>
        <?php $this->scripts(); ?>
        <?php
    }
    ...
    private function features(): void {
        ...
        $post_acf_controls = $this->inline_toggle_setting_html( "post_summary_acf_enabled", "Register Post Summary on posts" ) . $this->inline_toggle_setting_html( "post_faqs_acf_enabled", "Register Post FAQs on posts" ) . $this->post_acf_shortcode_reference_html();
    ...
    private function icon_style_setting_html( array $settings ): string {
        return $this->select_setting_html( "muckrack_icon_style", [ "circle_check" => [ "label" => "Circle with check", "description" => "Solid circle badge with a Font Awesome check. This is the default author verification badge.", "preview" => $this->author_icon_preview_html( $settings, "circle_check" ) ], "check" => [ "label" => "Just check", "description" => "Font Awesome check only, no circle background.", "preview" => $this->author_icon_preview_html( $settings, "check" ) ] ], $settings, "Icon style" );
    }
    ...
    private function author_icon_preview_html( array $settings, string $style = "" ): string {
        ...
        $class = "check" === $style ? "smpi-muckrack-preview-check" : "smpi-muckrack-preview-circle";
        return "<span class=\"" . esc_attr( $class ) . "\" style=\"--smpi-muckrack-color:" . esc_attr( $color ) . "\" aria-label=\"Verified by MuckRack editorial team\"><i class=\"fas fa-solid fa-check\" aria-hidden=\"true\"></i></span>";
    }
    ...
    private function author_muckrack_mode_help_html( array $settings ): string {
        ...
        $style = "check" === (string) ( $settings["muckrack_icon_style"] ?? "circle_check" ) ? "Font Awesome check only" : "solid circle badge with Font Awesome check";
    ...
    private function post_acf_addons_report_html(): string {
        ...
        $html .= "<tr><td>Post Summary</td><td>" . ( $summary_enabled ? "Enabled" : "Disabled" ) . "</td><td>" . $this->ico( (bool) $summary_registered ) . "</td><td><code>field_65ab7ba0e849b</code></td><td><code>post</code>, <code>imported-news</code></td></tr>";
        $html .= "<tr><td>Post FAQs</td><td>" . ( $faqs_enabled ? "Enabled" : "Disabled" ) . "</td><td>" . $this->ico( (bool) $faqs_registered ) . "</td><td><code>field_65ab7bc1e849c</code></td><td><code>post</code>, <code>imported-news</code></td></tr>";
    ...
    private function scripts(): void {
        ...
        jQuery(function($){
            ...
            $(window).on(`popstate`,function(){var params=new URLSearchParams(window.location.search),tab=params.get(`tab`)||`overview`;loadTab(tab,tabUrl(tab),false)});
