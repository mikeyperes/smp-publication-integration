...
[smp_post_authors context=\"card\" format=\"lines\"]", $this->multi_authors_report_html(), $this->activity_log_html(), $multi_author_controls );
    ...
    private function quick_start_item_html( string $item_id, array $item, bool $complete ): string {
        ...
        $html = "<article class=\"smpi-feature-card smpi-quick-start-item" . ( $complete ? " is-complete" : "" ) . "\" data-smpi-quick-start-item=\"" . esc_attr( $item_id ) . "\">";
        $html .= "<div class=smpi-feature-head><div><p class=smpi-kicker>Checklist item</p><h2>" . esc_html( $title ) . "</h2><p>" . esc_html( $description ) . "</p></div><div class=smpi-feature-toggle><span class=\"smpi-pill " . ( $complete ? "smpi-pill--saved" : "smpi-pill--pending" ) . "\">" . ( $complete ? "Configured" : "Needs apply" ) . "</span></div></div>";
        $html .= "<div class=smpi-feature-grid><section><h3>Target settings</h3><dl class=smpi-quick-start-details>";
        foreach ( (array) ( $item["details"] ?? [] ) as $detail ) {
            $label = (string) ( $detail["label"] ?? "" );
            $value = (string) ( $detail["value"] ?? "" );
            $color = isset( $detail["color"] ) ? sanitize_hex_color( (string) $detail["color"] ) : "";
            $html .= "<div><dt>" . esc_html( $label ) . "</dt><dd>";
            if ( "" !== $color ) {
                $html .= "<span class=smpi-color-swatch style=\"background:" . esc_attr( $color ) . "\"></span> ";
            }
            $html .= "<code>" . esc_html( $value ) . "</code></dd></div>";
        }
        $html .= "</dl></section><section><h3>Action</h3><p>Writes the exact saved values listed here into <code>smpi_settings</code>.</p><button type=button class=\"button " . ( $complete ? "button-secondary" : "button-primary" ) . "\" data-smpi-quick-start-apply=\"" . esc_attr( $item_id ) . "\">" . ( $complete ? "Reapply settings" : "Apply settings" ) . "</button><span class=spinner></span><span class=\"smpi-save-state\" aria-live=\"polite\"></span></section></div>";
        return $html . "</article>";
    }
...
[acf_author_field field=\"job_title\" author_index=\"1\"]", $this->multi_authors_report_html(), $this->activity_log_html(), $this->multi_author_controls_html( $settings ) );
    ...
    private function scripts(): void {
        ...
        jQuery(function($){
            ...
            $(document).on(`click`,`[data-smpi-quick-start-apply]`,function(){var b=$(this),item=b.data(`smpi-quick-start-apply`)||b.attr(`data-smpi-quick-start-apply`),r=saveRoot(b),card=featureRoot(b),label=item===`all`?`all Quick Start feature settings`:($.trim(card.find(`h2`).first().text())||item);r.find(`.spinner`).addClass(`is-active`);b.prop(`disabled`,true);setSaveState(r,`saving`,`Applying...`);setFeatureSaveState(card,`saving`,`Applying ${label}`);setGlobalSaveToast(`saving`,`Applying ${label}`);$(`.smpi-tab-message`).text(`Applying ${label}`);$.post(smpiAdmin.ajaxUrl,{action:`smpi_quick_start_apply`,nonce:smpiAdmin.nonce,item:item,tab:smpiAdmin.activeTab}).done(function(x){var ok=!!(x&&x.success),msg=saveMessage(x,`Server rejected the Quick Start action.`);setSaveState(r,ok?`saved`:`error`,ok?`✓ Applied`:msg);setFeatureSaveState(card,ok?`saved`:`error`,ok?msg:`Error applying ${label}: ${msg}`);setGlobalSaveToast(ok?`saved`:`error`,ok?msg:`Error applying ${label}: ${msg}`);if(ok&&x.data&&x.data.fragment){refreshActiveFragment(x.data.fragment)}$(`.smpi-tab-message`).text(ok?msg:`Error applying ${label}: ${msg}`)}).fail(function(xhr){var msg=`HTTP ${xhr.status||0} ${xhr.statusText||`request failed`}`;setSaveState(r,`error`,msg);setFeatureSaveState(card,`error`,`Error applying ${label}: ${msg}`);setGlobalSaveToast(`error`,`Error applying ${label}: ${msg}`);$(`.smpi-tab-message`).text(`Error applying ${label}: ${msg}`)}).always(function(){r.find(`.spinner`).removeClass(`is-active`);card.removeClass(`is-saving`);b.prop(`disabled`,false)})});
    ...
    private function styles(): void { ?>
        ...
        <style id="smpi-quick-start-css">.smpi-quick-start-summary{align-items:center;display:flex;justify-content:space-between;gap:18px}.smpi-quick-start-actions{align-items:center;display:flex;gap:10px;flex-wrap:wrap}.smpi-quick-start-list{display:grid;gap:16px}.smpi-quick-start-item .smpi-feature-head p{max-width:820px}.smpi-pill--pending{background:#fff8e5;color:#8a6100}.smpi-quick-start-details{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:10px;margin:0}.smpi-quick-start-details div{background:#fff;border:1px solid #e2e8f0;border-radius:10px;padding:10px}.smpi-quick-start-details dt{color:#475569;font-size:11px;font-weight:800;letter-spacing:.05em;margin:0 0 6px;text-transform:uppercase}.smpi-quick-start-details dd{align-items:center;display:flex;gap:8px;margin:0;min-width:0}.smpi-quick-start-details code{white-space:normal;word-break:break-word}.smpi-quick-start-details .smpi-color-swatch{height:24px;width:24px}.smpi-quick-start-item.is-complete{border-color:#b8e0c4}@media(max-width:782px){.smpi-quick-start-summary{align-items:flex-start;display:block}.smpi-quick-start-actions{margin-top:12px}}</style>
    <?php }
}
