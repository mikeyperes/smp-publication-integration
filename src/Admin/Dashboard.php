...
final class Dashboard {
    ...
    private function features(): void {
        ...
        $author_muckrack_controls = $this->author_muckrack_mode_help_html( $settings ) . $this->author_muckrack_shortcodes_html() . $this->context_select_html( "muckrack_verified_contexts", [ "single_author" => "single.php header author mention", "single_footer" => "single.php footer/about-author mention", "home" => "Home page author mention", "author" => "author.php author mention" ], $settings, "Placement contexts" ) . $this->select_setting_html( "muckrack_verified_style", [ "tooltip" => [ "label" => "Tooltip icon", "description" => "Small badge beside the author name. Hover or focus explains the verification without adding sentence-length text.", "preview" => $this->author_tooltip_preview_html( $settings ) ], "text" => [ "label" => "Inline text", "description" => "Writes the full verification sentence directly into the author area.", "preview" => $this->author_inline_preview_html( $settings ) ], "compact_block" => [ "label" => "Small editorial block", "description" => "Smaller author version of the editorial verification block with left accent and compact text.", "preview" => $this->author_compact_block_preview_html( $settings ) ] ], $settings, "Display style" ) . $this->icon_style_setting_html( $settings ) . $this->color_setting_html( "muckrack_icon_color", "Icon color", $settings ) . $this->number_setting_html( "muckrack_icon_size", "Checkmark size", $settings, 8, 64, "px" ) . $this->inline_toggle_setting_html( "muckrack_author_always_show", "Always show for every author" );
        ...
        $toc_controls = $this->inline_toggle_setting_html( "table_of_contents_auto_single", "Automatically show above single.php content" ) . $this->select_setting_html( "table_of_contents_style", $this->toc_style_options(), $settings, "Table of contents design" );
    ...
    private function number_setting_html( string $key, string $label, array $settings, int $min = 8, int $max = 64, string $suffix = "" ): string {
        $value = isset( $settings[ $key ] ) ? absint( $settings[ $key ] ) : $min;
    ...
    private function toc_design_preview_html( string $style ): string {
        return "<nav class=\"smpi-design-preview smpi-design-toc smpi-" . esc_attr( $style ) . "\"><span class=smpi-toc-label>In this article</span><ol><li><a href=#>The rise of new technology</a></li><li><a href=#>What the data reveals</a></li></ol></nav>";
    }

    private function inline_photo_preview_html( string $style ): string {
        return "<figure class=\"smpi-design-preview smpi-design-photo smpi-design-" . esc_attr( $style ) . "\"><span class=smpi-photo-block></span><figcaption>Collegiate summer leagues provide a platform for emerging talent.</figcaption></figure>";
    }
    ...
    private function faq_design_preview_html( string $style ): string {
        return "<section class=\"smpi-design-preview smpi-design-faq smpi-" . esc_attr( $style ) . "\"><h2>Frequently asked questions</h2><div><p><strong>What is this?</strong></p><p>A compact preview of the selected FAQ treatment.</p></div></section>";
    }
    ...
    private function author_icon_preview_html( array $settings, string $style = "" ): string {
        ...
        $size = isset( $settings['muckrack_icon_size'] ) ? absint( $settings['muckrack_icon_size'] ) : 18;
        $size = max( 8, min( 64, $size ?: 18 ) );
        return '<span class="' . esc_attr( $class ) . '" style="--smpi-muckrack-color:' . esc_attr( $color ) . ';color:' . esc_attr( $color ) . ';font-size:' . esc_attr( (string) $size ) . 'px" aria-label="Verified by MuckRack editorial team">' . $this->muckrack_icon_svg_html( $style ) . '</span>';
    }
