...
`Hexa\PluginCore\Typography\TemplateTypography` defines the shared three-mode contract: Template Default, Site Typography, and Custom Typography. Template Default emits no typography overrides. Site Typography inherits every supported property. Custom Typography applies only configured properties whose adjacent `Leave as is` toggle is off.
...
Optional font-style fields follow the font-family preservation setting. When `Leave font as is` is active, Core disables both the family selector and every associated style field so the UI matches the emitted CSS.
