...
For selectable visual templates, use `TemplateColorControl::render()` and `BrandColors\TemplateColorResolver`. The shared color modes are Template Default, Site Primary, Site Secondary, and Custom. Hosts register each template's native `accent` plus only the CSS variables for decorative surfaces that are allowed to change. Template Default emits no variables. Core resolves explicit picker and hex events before fallback synchronization so host save listeners receive the selected custom value regardless of script render order.
