...
(function(){
    ...
    function apply(control,mode,color,template){if(!control)return{};if(template!==undefined&&template!==null)control.setAttribute("data-hpc-template-color-template",String(template));mode=mode||source(control);syncCustomDisplay(control);color=hex(color)||base(control,mode);var variables=json(control,"data-hpc-template-color-variables"),host=scope(control),resolved={};Object.keys(variables).forEach(function(variable){if(!/^--[a-z0-9_-]+$/.test(variable))return;var value=mode===DEFAULT?"":transform(color,variables[variable]);if(host){if(value)host.style.setProperty(variable,value);else host.style.removeProperty(variable)}if(value)resolved[variable]=value});control.setAttribute("data-hpc-template-color-source",mode);var customPanel=control.querySelector("[data-hpc-template-color-custom]");if(customPanel)customPanel.hidden=mode!==CUSTOM;updateSwatches(control);document.dispatchEvent(new CustomEvent("hexa-template-color-change",{detail:{control:control,source:mode,color:color,template:control.getAttribute("data-hpc-template-color-template")||"",variables:resolved,scope:host}}));return resolved}
