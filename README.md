...
- Kept Elementor's Table of Contents implementation native while replacing its oversized spinner and nested scroll area with a compact CSS-only loading state.
- Prevented Elementor sticky-header spacer clones and repeated submenu indicators from exposing duplicate navigation arrows.
- Refined the Eyebrow Bullets Summary template and added Template, None, and Custom background modes backed by the Hexa Core color picker.

## 0.6.251 Updates

- Added an Article reading progress feature with five selectable top-of-page designs and a reusable Hexa Core color picker.
- Limited progress output to enabled public single posts and added passive, animation-frame-based scroll tracking with accessible progress semantics.

## 0.6.250 Updates

- Restored native Elementor ownership of Elementor Table of Contents markup, loading state, and anchors by removing SMP's competing fallback bridge and global Elementor TOC overrides.
- Kept SMP's shortcode and automatic Table of Contents implementations isolated to SMP-owned markup.
...
- Preserved real image dimensions when publication schema falls back to a WordPress site icon or a local attachment URL.
...
- Updated the shared template controls to use clear Original Template, site-value, and custom-value wording with consistently left-aligned inheritance toggles and visibly muted inherited editors.
- Gave Article Summary templates canonical native body typography so Original Template renders identically in admin previews and on the frontend while design colors remain independently selectable.
...
- Updated Hexa WP Core to `0.19.75` so every shared template-color picker persists the newly selected custom value regardless of Core script render order.
- Added regression coverage for the exact stale-value event sequence across SMP's shared design controls.
...
- Removed obsolete hardcoded Summary and FAQ admin-preview rules so the shared article template stylesheet is the only source for those designs.

## 0.6.240 Updates

- Changed Summary color customization to control each template's design accent while article text continues to inherit the active theme.
- Set the Key Points Card's left corners square and made Summary and Numbered FAQ accent treatments share live-preview and frontend variables.
- Added Manual, Below content, and Below author FAQ placement modes using the existing FAQ shortcode renderer.

## 0.6.239 Updates

- Prevented the dynamic article-markup fallback from assigning a second drop cap when server-rendered content already identifies the lead paragraph.

## 0.6.238 Updates

- Updated Hexa WP Core to `0.19.73` so preserved color settings disable the full picker and import controls while keeping the adjacent preservation toggle available.

## 0.6.237 Updates

- Centralized typography controls for all ten design surfaces through the reusable Hexa WP Core control.
- Added adjacent font, size, color, and weight preservation toggles with AJAX persistence, live preview updates, and frontend-aware CSS output.
- Added complete typography settings and reporting for breadcrumbs, summaries, and author and publication verification output.

## 0.6.236 Updates

- Updated bundled Hexa WP Core to `0.19.72`, adding a light shared highlight to every expanded feature card.

## 0.6.235 Updates

- Updated the bundled Core typography control to `0.19.71` so the color-preservation toggle stays attached to the color heading at every width.

## 0.6.234 Updates

- Replaced the detached heading and drop-cap preservation panels with Hexa WP Core `0.19.70` combined typography controls, placing every preservation toggle beside its corresponding field.

## 0.6.233 Updates

- Split the five admin script-preview fonts into independent stylesheet requests so Cloudflare loads every family instead of rendering fallback glyphs after the first family.

## 0.6.232 Updates

- Changed drop-cap previews to use a visibly calligraphic `F` glyph across all templates so the five script families are clearly distinguishable.
...
- Updated the bundled Hexa WP Core package to `0.19.68` and added its optional font-weight selector to all ten SMP design font controls.
- Added Dancing Script, Great Vibes, Parisienne, and Pinyon Script choices to the article drop-cap feature, with selected-font-only frontend loading.
- Added four default-on Article H2/H3 controls that preserve the theme font family, size, color, and weight while retaining the selected decoration.
- Changed top-level Shortcodes cards to load collapsed and preserve expanded state through Hexa WP Core's `hpc_open` query state.

## 0.6.229 Updates

- Updated the bundled Hexa WP Core package to `0.19.67`.
- Added the reusable Core font selector to all ten SMP design outputs.
- Saved Elementor typography source IDs and applied them to admin previews and frontend output without loading duplicate fonts.

## 0.6.222 Updates

- Updated the bundled Hexa WP Core package to 0.19.66.
- Preserved multiple expanded feature cards across WordPress admin refreshes with one canonical query value.

## 0.6.221 Updates

- Updated the bundled Hexa WP Core package to 0.19.65.
- Kept explicit query-string state authoritative for default-open cards during initial and AJAX rendering.

## 0.6.220 Updates

- Updated the bundled Hexa WP Core package to 0.19.64.
- Feature cards now preserve expanded state through the page query string and restore it after refresh.
...
- Replaced duplicated author-context color markup and JavaScript with the reusable Hexa WP Core color control.
- Added editable hex values, HWS primary import, and explicit inherited/custom state to every author context.
- Replaced Breadcrumb visibility checkboxes with reusable Hexa WP Core toggles.
- Added an independently saved option to hide breadcrumbs on standard single-post pages.
- Updated the bundled Hexa WP Core package to 0.19.63.

## 0.6.216 Updates

- Removed the team-member directory feature that was incorrectly added to this plugin in 0.6.215.
- Restored ownership of team-member directory templates and their shortcode to HWS Base Tools.

## 0.6.214 Updates

- Scoped Features search to each card title, description, and settings so shared activity logs cannot create false matches.
- Updated the bundled Hexa WP Core package to 0.19.54.
...
- Added reusable plugin and Core version identity to the Hexa WP Core sidebar, with safe GitHub links and collapsed-state support.
- Moved Quick Start directly below Dashboard and renamed the Editorial navigation group to Articles while preserving legacy routes.
- Made the saved breadcrumb background own a full-width frontend and preview band, and removed the forced light divider from Flush Minimal.
- Preserved existing breadcrumb CSS overrides while adding the reusable full-band override scope.

## 0.6.209 Updates

- Added stable shared and component classes to Breadcrumbs, table of contents, article content, inline images, featured captions, summaries, and FAQs.
- Applied the same class contract in PHP renderers, dynamic-render JavaScript fallbacks, and Features-tab previews.
- Moved frontend design CSS off Rank Math, Elementor, bare child tags, and template-specific markup dependencies.
- Added regression coverage for every template render path and the shared WordPress HTML processor.

## 0.6.208 Updates

- Updated the bundled Hexa WP Core to 0.19.49.
- Replaced the wrapped tab bar with a grouped sidebar powered by the Core host-tabs renderer.
- Preserved all 23 flat tab routes while organizing them into publication areas.

## 0.6.207 Updates

- Organized the Features tab into Article design, Authors and verification, Content and distribution, and System integrations.
- Moved settings ahead of collapsed implementation reference, diagnostics, and Activity Log panels.
- Preserved all feature controls, shortcodes, reports, and the Breadcrumb CSS override editor.

## 0.6.206 Updates

- Updated the vendored Hexa WP Core to 0.19.48.
- Added a saved Breadcrumb CSS editor with scoped validation and AJAX feedback.
- Appended validated overrides after the selected Breadcrumb template CSS.

## 0.6.205 Updates

- Updated the vendored Hexa WP Core to 0.19.47.
- Added the Core `ScopedCssOverride` reference above the Breadcrumbs Activity Log.
- Added a leak-resistant breadcrumb selector with formatted HTML and CSS examples.

## 0.6.204 Updates

- Changed Breadcrumbs templates to one full-width preview per row at every viewport.

## 0.6.203 Updates

- Reorganized Breadcrumbs settings into Template, Appearance, and Visibility sections.
- Grouped home, archive, and custom-post-type visibility controls together.
- Added compact two-column template and exclusion grids with a single-column mobile layout.

## 0.6.202 Updates

- Rendered every Features card through the Hexa WP Core collapsible component and made all cards closed by default.
- Added a breadcrumb background color control applied across every breadcrumb template and live admin preview.

## 0.6.201 Updates

- Added WP User Avatars to the explicit forbidden plugin policy.
- Installed forbidden plugins expose Core AJAX Deactivate and Delete actions.

## 0.6.200 Updates

- Removed Under Construction from the required plugin policy.
- Registered SMP WP Text To Speech against its GitHub repository so missing installations use the Core AJAX install-and-activate action.

## 0.6.199 Updates

- Moved Simple Local Avatars into Required Plugins with its WordPress.org AJAX install-and-activate action.
- Restricted Forbidden Plugins to JetEngine and hid absent forbidden definitions from the violations table.

## 0.6.198 Updates

- Updated the vendored Hexa WP Core to 0.19.46.
- Removed the separate Source column from Required and Forbidden plugin inventories; source now appears beneath the plugin path.
- Added a fixed seven-column desktop layout and labeled mobile rows without horizontal scrolling.

## 0.6.197 Updates

- Updated the vendored Hexa WP Core to 0.19.45.
- Kept AJAX-refreshed plugin rows free of duplicate Core and DynamicButton style/script tags.
